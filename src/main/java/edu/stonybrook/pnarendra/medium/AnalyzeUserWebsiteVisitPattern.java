...
class VisitDetail {
	...
	public VisitDetail(int timestamp, String username, String url) {
		super();
...
public class AnalyzeUserWebsiteVisitPattern {
	...
	public List<String> mostVisitedPattern(String[] user, int[] time, String[] web) {
		...
		// As we need only the top 1 element using priority queue so that the time
	...
	public HashMap<String, Integer> fillThreeSeq(HashMap<String, List<String>> userWebsites) {
		...
		for (String user : userWebsites.keySet()) {
			...
			if (websites.size() >= 3) {
				// getThreeCom is used to get the list of all possible 3-sequence websites with
				// each 3-sequence considered only once for one user
	...
	public HashSet<String> getThreeComb(List<String> websites) {
		...
		helperFun(sol, new LinkedList<String>(), 0, websites);
		return sol;
	}
	...
	public void helperFun(HashSet<String> sol, List<String> cur, int pos, List<String> websites) {
		if (cur.size() == seqLen) {
			...
			// Setting the 3-sequence as a string with each website name separated by a
			// space
	...
	public static void main(String[] args) {
		...
		int[] timestamp = { 527896567, 334462937, 517687281, 134127993, 859112386, 159548699, 51100299, 444082139,
				926837079, 317455832, 411747930 };
		String[] website = { "hibympufi", "hibympufi", "hibympufi", "hibympufi", "hibympufi", "hibympufi", "hibympufi",
				"hibympufi", "yljmntrclw", "hibympufi", "yljmntrclw" };
		new AnalyzeUserWebsiteVisitPattern().mostVisitedPattern(username, timestamp, website);
